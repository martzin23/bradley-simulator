...
@export var scene: PackedScene:
	set(value):
		if (value):
			var scene_instance = value.instantiate()
			var mesh_instance = scene_instance.find_children("*", "MeshInstance3D", true, false).front()
			...
			push_error("No valid mesh found in provided scene!")
...
@export_range(0.125, 256.0, 0.015625) var instance_spacing: float = 0.5:
	set(value):
		instance_spacing = clamp(round(value * 64.0) * 0.015625, 0.125, 256.0)
...
func _create_grid() -> void:
	...
	var half_grid: int = int(grid_width / 2.)
